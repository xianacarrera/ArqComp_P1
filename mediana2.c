...
int main(int argc, char * argv[]){
    ...
    long R[NL*ND];
    long N[NL*ND];
    /* Matriz de tiempos por acceso
     * Cada columna se corresponde a un experimento para el que se han realizado NL*ND = 35 medidas
     */
    double CK[NL*ND][NE]; 
    ...
    // Lectura de los 350 experimentos realizados, 10 experimentos para cada par (D,L)
    for (j = 0; j < NE; j++){
    ...
    for (i = 0; i < NL * ND; i++){
	// Ordenación de los 10 tiempos de acceso obtenidos para cada par (D,L) para la obtención de la mediana
        sort(CK[i]);
